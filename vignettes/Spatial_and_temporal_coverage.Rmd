...
```{r general_characteristics, results = 'asis'}
library(knitr)
library(QuickUDS)
library(dplyr)
...
```{r kailitz_classification_problems_1, results='asis'}
...
```{r kailitz_classification_problems_2, results='asis'}
print(kable(kailitz.yearly %>% 
        filter(grepl("-",combined_regime)) %>%
        group_by(country_name) %>%
        arrange(country_name,year) %>%
...
```{r kailitz_index_count, results = 'asis'}
print(kable(kailitz.yearly %>%
